...
import { type LoggingContextController, type PerformanceMetrics } from '@shared/rest-controller.types'
import { type SensitiveDataSanitizer } from '@shared/sensitive-data-sanitizer'
...
export class PerformanceTracker implements IPerformanceTracker {
  ...
  constructor(
    ...
    readonly options: PerformanceTrackerOptions
  ) {
    this.enableLogging = options.enableLogging
    this.logSlowRequests = options.logSlowRequests
    this.slowRequestThresholdMs = options.slowRequestThresholdMs
    this.includeRequestData = options.includeRequestData
    this.includeResponseData = options.includeResponseData
  }
  ...
  public trackRequest<TRequest, TResponse>(parameters: {
    ...
    const metrics: PerformanceMetrics<TRequest, TResponse> = {
      ...
      endTime,
      runtimeMs,
      success: parameters.isSuccess,
      controllerName: parameters.context.controllerName
    }
    this.logMetrics({ metrics, context: parameters.context })
    if (this.logSlowRequests && runtimeMs > this.slowRequestThresholdMs) {
      this.logSlowRequest({ metrics, context: parameters.context })
    }
  }

  private logMetrics<TRequest, TResponse>(parameters: {
    metrics: PerformanceMetrics<TRequest, TResponse>
    context: LoggingContextController
  }): void {
    const sanitizedRequest = this.includeRequestData
      ? this.dataSanitizer.sanitize({ data: parameters.metrics.request })
      : '[REQUEST_LOGGING_DISABLED]'
    const sanitizedResponse = this.includeResponseData
      ? this.dataSanitizer.sanitize({ data: parameters.metrics.response })
      : '[RESPONSE_LOGGING_DISABLED]'
  ...
  private logSlowRequest<TRequest, TResponse>(parameters: {
    metrics: PerformanceMetrics<TRequest, TResponse>
    context: LoggingContextController
  }): void {
    ...
    this.loggerProvider.sendLogTimeController({
      ...
      controllerName: parameters.metrics.controllerName,
      runtimeInMs: parameters.metrics.runtimeMs,
      httpRequest: this.dataSanitizer.sanitize({ data: parameters.metrics.request }),
      httpResponse: this.dataSanitizer.sanitize({ data: parameters.metrics.response }),
      isSuccess: parameters.metrics.success,
      context: parameters.context
    })
  }

  private buildLogMessage<TRequest, TResponse>(parameters: {
    metrics: PerformanceMetrics<TRequest, TResponse>
    context: LoggingContextController
  }): string {
    ...
    if (parameters.context.requestID.toString()) message += ` [RequestID: ${parameters.context.requestID.toString()}]`
    return message
  }

  public static createDefault(parameters: {
    loggerProvider: ISendLogTimeControllerLoggerProvider
    dataSanitizer: SensitiveDataSanitizer
  }): PerformanceTracker {
    return new PerformanceTracker(parameters.loggerProvider, parameters.dataSanitizer, {
      enableLogging: true,
  ...
  public static createProductionOptimized(parameters: {
    loggerProvider: ISendLogTimeControllerLoggerProvider
    dataSanitizer: SensitiveDataSanitizer
  }): PerformanceTracker {
    return new PerformanceTracker(parameters.loggerProvider, parameters.dataSanitizer, {
